...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if count == 0:
            logger.info("🧾 No restaurant data found. Initializing from JSON...")
            await data_preprocessor.initialize_data()
    except Exception as e:
        logger.warning(f"⚠️  Failed to initialize restaurant data: {e}")
    ...
    if not faiss_loaded:
        ...
        try:
            ...
            faiss_loaded = vector_store.index is not None
            if faiss_loaded:
                logger.info("  ✓ FAISS index built")
        except Exception as e:
            logger.warning(f"  ⚠️  FAISS index build failed: {e}")

    if not bm25_loaded:
        logger.info("  🧱 Building BM25 index from database...")
        try:
            await bm25_search.build_index(force_rebuild=True)
            bm25_loaded = bm25_search.bm25 is not None
            if bm25_loaded:
                logger.info("  ✓ BM25 index built")
        except Exception as e:
            logger.warning(f"  ⚠️  BM25 index build failed: {e}")

    if not faiss_loaded or not bm25_loaded:
        logger.warning("  ⚠️  Search indexes are incomplete; recommendations may be limited")
...
if extra_origins:
    ALLOWED_ORIGINS_RAW.extend([origin.strip() for origin in re.split(r"[;,]", extra_origins) if origin.strip()])
