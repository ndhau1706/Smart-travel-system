...
export function mapApiRestaurantToFrontend(apiRestaurant: ApiRestaurant): Restaurant {
  ...
  return {
    ...
    googleMapsUrl: apiRestaurant.google_maps_url || "",
    menu,
  };
}
...
// API Functions
export type RestaurantSortBy = "rating" | "price" | "name";
export type RestaurantSortOrder = "asc" | "desc";

export interface FetchRestaurantsPageParams {
  page?: number;
  limit?: number;
  search?: string;
  cuisine?: string;
...
export interface RestaurantsPageResult {
  restaurants: Restaurant[];
  pagination: PaginationMeta | null;
}

export async function fetchRestaurantsPage(
  params: FetchRestaurantsPageParams = {},
): Promise<RestaurantsPageResult> {
  try {
    ...
    if (typeof params.price_level === "number") query.set("price_level", String(params.price_level));
    if (typeof params.rating === "number") query.set("rating", String(params.rating));
    if (params.sort_by) query.set("sort_by", params.sort_by);
    if (params.sort_order) query.set("sort_order", params.sort_order);

    const response = await fetch(`${API_URL}/restaurants?${query.toString()}`);
    if (!response.ok) {
    ...
    return { restaurants: [], pagination: null };
  } catch (error) {
...
export async function recommendRestaurantsForChat(
  ...
  const response = await fetch(`${API_URL}/chat/recommend`, {
    ...
    body: JSON.stringify({ message, limit, history }),
  });
